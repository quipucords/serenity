...
def create_dd_df():
    ...
    deployment_details_columns = ['Source', 'Product_Installed', 'Product_Version', 'RH_Packages_Installed',
                                  'Operating_System_Hostname', 'VM_Name', 'Physical/Virtual', 'Physical_Sockets',
                                  'Socket_Pair', 'Cores/vCPU_Count', 'If_Virtual,Hosted_by', 'If_Virtual,Sockets_on_Host', 'If_Virtual,_Host_Cluster_name',
                                  'Install_Date/Profile_Creation_Date','Decomm_Date/Last_Active_date', 'RHEL_ELS?',
                                  'Jboss_Type', 'Hyperthreading(True/False)', 'Ansible/OpenShift',
                                  'RHEL_Group_(Instance/Host based)', 'Duplicate?(Y/N)',
                                  'Considered?(Y/N)', 'Comments']
...
def fill_deployment_from_vcenter(deployment_df, vcenter_data):
    ...
    if not vcenter_data.empty:
        ...
        deployment_df['Operating_System_Hostname'] = vcenter_data.get('vm.dns_name', '-')
        ...
        deployment_df['If_Virtual,Sockets_on_Host'] = vcenter_data.get('vm.host.socket_count', '-')
        deployment_df['If_Virtual,_Host_Cluster_name'] = vcenter_data.get('vm.cluster', '-')
...
def fill_deployment_from_network(deployment_df, network_data):
    ...
    if not network_data.empty:
        ...
        deployment_df['RH_Packages_Installed'] = network_data.get('redhat_packages_gpg_num_rh_packages', '-')
        deployment_df['Operating_System_Hostname'] = network_data.get('uname_hostname', '-')
        ...
        deployment_df['Socket_Pair'] = np.ceil(deployment_df['Physical_Sockets'].replace('-', '0').astype(float) / 2)
        ...
        deployment_df['Install_Date/Profile_Creation_Date'] = network_data.get('Creation Date / Install date', '-')
        deployment_df['Decomm_Date/Last_Active_date'] = date.today()
        deployment_df['RHEL_ELS?'] = '-'
        deployment_df['Jboss_Type'] = '-'
        deployment_df['Hyperthreading(True/False)'] = '-'
...
def fill_deployment_from_satellite(deployment_df, satellite_data):
    ...
    if not satellite_data.empty:
        ...
        deployment_df['If_Virtual,Hosted_by'] = satellite_data.get('virtual_host_name', '-')
        deployment_df['If_Virtual,Sockets_on_Host'] = '-'
        deployment_df['If_Virtual,_Host_Cluster_name'] = '-'
        deployment_df['Install_Date/Profile_Creation_Date'] = satellite_data.get('install_date', '-')
        deployment_df['Decomm_Date/Last_Active_date'] = date.today()
        deployment_df['RHEL_ELS?'] = '-'
        deployment_df['Jboss_Type'] = '-'
        deployment_df['Hyperthreading(True/False)'] = '-'
