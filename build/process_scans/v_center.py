...
def trim_cm_dns_name(v_center_df):
    ...
    v_center_df['vm.dns_name_trimmed'] = v_center_df['vm.dns_name'].apply(lambda x: x.split('.', 1)[0])
...
def add_considered_comment(v_center_df):
    ...
    v_center_df['Considered'] = v_center_df['vm.os'].apply(lambda x: 'Y' if 'Red Hat Enterprise Linux' in x else 'N')

    # Create the 'Comment' column
    v_center_df['Comment'] = v_center_df.apply(lambda row: 'Non Red Hat Server' if row['Considered'] == 'N' else '', axis=1)
...
def split_rh_nonrh(v_center_df):
    ...
    non_red_hat_servers = v_center_df[v_center_df['Comment'] == 'Non Red Hat Server'].copy()
...
def ignore_template_and_discovery(red_hat_servers):
    ...
    red_hat_servers['Considered'] = red_hat_servers.apply(lambda row: 'N' if 'template' in row['vm.name'].lower() or 'template' in row['vm.dns_name_trimmed'].lower() or 'discovery' in row['vm.name'].lower() or 'discovery' in row['vm.dns_name_trimmed'].lower() else row['Considered'], axis=1)

    # Update the 'Comment' column
    red_hat_servers['Comment'] = red_hat_servers.apply(lambda row: 'Template or Discovery server' if 'template' in row['vm.name'].lower() or 'template' in row['vm.dns_name_trimmed'].lower() or 'discovery' in row['vm.name'].lower() or 'discovery' in row['vm.dns_name_trimmed'].lower() else row['Comment'], axis=1)
...
def duplicate_strategy_1(red_hat_servers):
    ...
    red_hat_servers['Same'] = red_hat_servers.duplicated(subset='vm.dns_name_trimmed') & (red_hat_servers['vm.dns_name_trimmed'] != 'localhost')
...
def duplicate_strategy_2(red_hat_servers): 
    ...
    for _, row in duplicate_rows.iterrows():
        ...
        same_dns_rows = red_hat_servers[red_hat_servers['vm.dns_name_trimmed'] == dns_name]
        ...
        if same_dns_rows['vm.name'].nunique() == 1:
            ...
            p_on_index_list = list(same_dns_rows[same_dns_rows['vm.state'] == 'poweredOn'].index)
            ...
            # If both are powered off consider any
                ...
                same_dns_rows.loc[same_dns_rows.index[0], 'Comment'] = 'Duplicate Considering any one'
            ...
            # If one of them is powered on and others are powered off, consider the one that is powered on
                ...
                same_dns_rows.loc[p_on_index_list[0], 'Comment'] = 'Duplicate Considering only Powered On'
...
def duplicate_strategy_3(red_hat_servers): 
    ...
    for index, row in red_hat_servers.iterrows():
        ...
        if 'test' in name or 'clone' in name or 'new' in name:
            ...
            original_name = name.replace('_test', '').replace('_clone', '').replace('_new', '').replace('_old', '').replace('old', '').replace('test', '').replace('clone', '').replace('new', '')
            similar_rows = red_hat_servers[red_hat_servers['vm.name'].str.contains(original_name)]
        ...
        for group_name, group_df in grouped:
            ...
            if len(group_df) > 1:
                ...
                if len(states) == 1 and states[0] == 'poweredOn':
                    ...
                    # Keep all servers
                    print(f"Group '{group_name}' has {len(group_df)} rows with 'vm.state' as 'poweredOn'")
                    print("Keeping all servers since Powered On")
                    red_hat_servers.loc[group_df.index, 'Same'] = True
                    red_hat_servers.loc[group_df.index, 'Comment'] = "Duplicate test, new, clone"
                    ...
                    print(f"Group '{group_name}' has at least one row with 'vm.state' as 'poweredOn'")
                    print("Keeping the server which is Powered On")
                    p_on_index_list = list(group_df[group_df['vm.state'] == 'poweredOn'].index)
                    ...
                    print(f"Group '{group_name}' has all rows with 'vm.state' as 'poweredOff'")
...
def product_name_version(v_center_df):
    ...
    v_center_df[['Product Name', 'Version']] = v_center_df['vm.os'].str.extract(r'^(.*?)\s*(\(.*\)|\d.*)?$')
    ...
    v_center_df.loc[mask1, 'Product Name'] = v_center_df.loc[mask1, 'vm.os'].str.extract(r'(Microsoft Windows).*?(XP.*)', expand=False)[0]
    v_center_df.loc[mask1, 'Version'] = v_center_df.loc[mask1, 'vm.os'].str.extract(r'(Microsoft Windows).*?(XP.*)', expand=False)[1]
    ...
    v_center_df.loc[mask2, 'Version'] = v_center_df.loc[mask2, 'vm.os'].str.extract(r'Other(.*)')[0]
...
def reorder_based_on(gbd_keys, auto_keys, gbd, auto):
    ...
    if merged_df.empty:
        raise ValueError("Rows in G data and Auto generated data cannot be ordered the same way. Comparison results are invalid")
...
def compare_dfs(gbd_keys, auto_keys, gbd_col, auto_col, gbd, auto):
    ...
    mismatched_rows_auto = merged[merged[auto_col+'_auto'] != merged[gbd_col+'_gbd']][auto_col+'_auto']
    mismatched_rows_gbd = merged[merged[auto_col+'_auto'] != merged[gbd_col+'_gbd']][gbd_col+'_gbd']
