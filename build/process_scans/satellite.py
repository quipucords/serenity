...
def trim_hostname(satellite_df):
    ...
    satellite_df['hostname_trimmed'] = satellite_df['hostname'].apply(lambda x: x.split('.', 1)[0])
...
def get_install_date(satellite_df):
    ...
    satellite_df['install_date'] = pd.to_datetime(satellite_df['registration_time'], format='%Y-%m-%d %H:%M:%S UTC', errors='coerce')
...
def virtwho_check(satellite_df):
    ...
    satellite_df['Considered'] = satellite_df['hostname_trimmed'].map(lambda x: 'N' if 'virt-who' in x else '')
    # Create the 'Comment' column
    satellite_df['Comment'] = satellite_df['hostname_trimmed'].map(lambda x: 'hypervisor' if 'virt-who' in x else '')
...
def check_server_type(satellite_df):
    ...
    satellite_df['phy_vir'] = satellite_df.apply(lambda x: 'Physical' if x['virt_type'] in ['not applicable', '-'] else 'Virtual', axis=1)
    satellite_df['phy_vir'] = satellite_df.apply(lambda x: 'hypervisor' if 'virt-who' in x['hostname_trimmed'] else x['phy_vir'], axis=1)
...
def check_missing_servers(satellite_df):
    ...
    satellite_df['Comment'] = satellite_df.apply(lambda x: 'details missing for server' if (x['uuid']=='-' and x['os_name']=='-') else x['Comment'], axis=1)
    satellite_df['Considered'] = satellite_df.apply(lambda x: 'N' if x['Comment']=='details missing for server' else x['Considered'], axis=1)
...
def check_nonrhel_servers(satellite_df):
    ...
    satellite_df['Comment'] = satellite_df.apply(lambda x: 'Non-RHEL' if x['os_name'] not in ('RHEL', 'RedHat', 'RedHat_Workstation', '-', 'Redhat') else x['Comment'], axis=1)
    satellite_df['Considered'] = satellite_df.apply(lambda x: 'N' if x['Comment']=='Non-RHEL' else x['Considered'], axis=1)
...
def check_duplicates_satellite(satellite_df, intermediate_vcenter_df):
    ...
    satellite_df['Duplicate'] = satellite_df['hostname_trimmed'].isin(intermediate_vcenter_df['vm.dns_name_trimmed']).map({True: 'Y', False: 'N'})
    
    satellite_df['Comment'] = satellite_df.apply(lambda x: 'Duplicate (reported in vcenter)' if x['Duplicate']=='Y' else x['Comment'], axis=1)
    
    satellite_df['Considered'] = satellite_df.apply(lambda x: 'N' if x['Duplicate']=='Y' else x['Considered'], axis=1)
...
def identify_physical_servers(satellite_df):
    ...
    satellite_df['Comment'] = satellite_df.apply(lambda x: 'Assumed as Physical (2-Socket) ' if x['Considered']=='Y' and (x['num_sockets'] in ('1', '-')) and (x['is_virtualized'] in ('Unknown', '-')) else x['Comment'], axis=1)
